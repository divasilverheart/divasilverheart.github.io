...
class GameExperience(object):
    ...
    def __init__(self, model, max_memory=100, discount=0.95):
        ...
        # deque with maxlen automatically discards the oldest episode when full,
        # avoiding the O(n) cost of deleting from the front of a list
    ...
    def get_data(self, data_size=10):
        ...
        for i, sample_idx in enumerate(np.random.choice(range(mem_size), data_size, replace=False)):
            envstate, action, reward, envstate_next, game_over = self.memory[sample_idx]
            inputs[i] = envstate
